...
export async function printThermalReceipt(data: ReceiptData) {
  ...
  const receiptHtml = `
    ...
    <html>
      ...
      <body>
        ...
        ${data.notes ? `<div class="row" style="margin-top: 2px;"><span style="font-weight: bold;">Note</span><span>${data.notes}</span></div>` : ''}
  ...
  if (isAndroid || settings.connectionType === 'rawbt') {
    ...
    if (data.notes) {
      text += `Note: ${String(data.notes).substring(0, 64)}\n`
    }
    text += `--------------------------------\n`
